...
def startsession():
    ...
    click.echo(
        click.style("Logged in successfully as {}".format(p["user_name"]), fg="green")
    )
...
def savecreds():
    ...
    click.echo(
        "Saves your creds in app config folder in file named {}".format(cred_file)
    )
    user_id = click.prompt("User ID >")
    password = click.prompt("Password >", hide_input=True)
    twofa = click.prompt("Pin >", hide_input=True)
    config = configparser.ConfigParser()
    config["CREDENTIALS"] = {"user_id": user_id, "password": password, "twofa": twofa}
...
def rm(config):
    ...
    if config == "CREDENTIALS":
        try:
            ...
            click.echo(
                "Could not find any credentials, you can save it again using 'jtrader zerodha savecreds'"
            )
        else:
            click.echo(
    ...
    if config == "SESSION":
        try:
            ...
            click.echo(
                "Could not find the session config, you can save it again using 'jtrader zerodha startsession'"
            )
        else:
            click.echo(
                "Successfully deleted, you can save it again using 'jtrader zerodha startsession'"
            )
